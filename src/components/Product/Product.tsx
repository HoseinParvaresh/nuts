...
export default function Product({ productData }: { productData: Product }) {
	...
	const renderItem = (item : any) => {
		return (
		  <div>
			<img
			  src={item.original || null}
			  alt=""
			  className={productData.main ? "rounded-sm" : "!h-60 2xs:!h-90 xs:!h-110 !w-120 rounded-full"}
			/>
		  </div>
		);
	  };
	...
	return (
		<>
			<div className="bg-white min-h-[418px] relative rounded-2xl shadow-md hover:shadow-xl transition-all duration-300 hover:-translate-y-1 border border-gray-100 overflow-hidden">
				<img src={productData.images[0].image1} className={` w-full object-cover object-top bg-gray-50 hover:scale-110 transition-transform ${productData.main ? "rounded-none" : "rounded-full"} h-[271px]`} />
				...
				<div className="fixed inset-0 bg-black/30 bg-opacity-50 flex items-center justify-center z-50">
					<div className="bg-white rounded-lg p-4 w-100 3xs:w-160 mx-4 max-h-[80vh] overflow-auto">
						...
						<div className="w-full">
							<ImageGallery items={images} renderItem={renderItem} showThumbnails={true} thumbnailPosition="right" showPlayButton={false} showFullscreenButton={false}  />
						</div>
					</div>
				</div>
			)}
