...
export const columns = (toggleShow: (id: string, show: boolean) => void, removeProduct: (id: string) => void, editProduct: (id: string, title: string, description: string, image1: File | string | undefined, image2: File | string | undefined, image3: File | string | undefined, image4: File | string | undefined, image5: File | string | undefined, show: boolean) => void): ColumnDef<Product>[] => [
	{
		accessorKey: "id",
		header: "شناسه",
	...
	},
	{
		...
		cell: ({ row }) => {
			...
			if (images[3]?.image4) {
				return (
					<div className="flex gap-2 size-20">
						<img src={`${images[3].image4}`} alt="product" className="object-cover rounded" />
					</div>
				);
			} else {
				return <div></div>;
	...
	},
	{
		...
		cell: ({ row }) => {
			...
			if (images[4]?.image5) {
				return (
					<div className="flex gap-2 size-20">
						<img src={`${images[4].image5}`} alt="product" className="object-cover rounded" />
					</div>
				);
			} else {
				return <div></div>;
	...
	},
	{
		...
		cell: ({ row }) => {
			...
			const [image4, setImage4] = React.useState<File | string | undefined>(product.images[3]?.image4);
			const [image5, setImage5] = React.useState<File | string | undefined>(product.images[4]?.image5);
								...
								<DialogFooter>
									...
									<Button
										...
										onClick={async (event) => {
											...
											setIsEditing(true);
											try {
												await editProduct(product.id, title, description, image1, image2, image3, image4, image5, product.show);
											} finally {
												setIsEditing(false);
											}
										}}
...
export function ProductsTable({ data }: { data: Product[] }) {
	...
	const editProduct = async (id: string, title: string, description: string, image1: File | string | undefined, image2: File | string | undefined, image3: File | string | undefined, image4: File | string | undefined, image5: File | string | undefined, show: boolean) => {
		let uploadedImages = [
			{ image1, primary: true },
